...
# Start dev server and check page source
pnpm run dev &
sleep 3
curl -s http://localhost:4321/ | grep -o "application/ld+json" | head -1 || echo "Schema not on non-homepage (expected)"
# Note: Current index.astro doesn't set isHomepage=true yet
kill %1
```
  </verify>
  <done>BaseLayout.astro includes JSON-LD Organization and WebSite schemas, conditionally rendered when isHomepage=true, TypeScript check passes</done>
</task>

<task type="auto">
  <name>Task 2: Add Playwright test for JSON-LD schemas</name>
...
4. tests/seo.spec.ts created with schema validation tests
5. TypeScript check passes: pnpm run check
</success_criteria>
